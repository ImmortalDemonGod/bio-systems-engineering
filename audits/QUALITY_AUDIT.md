...
| `physics/metrics.py` | Robustness | `compute_training_zones()` uses `np.isnan()` inside `apply()`. If HR/pace contain `None` (object dtype), `np.isnan(None)` raises, and zone classification can crash. | Low | Coerce to numeric (`pd.to_numeric(..., errors='coerce')`) before classification or guard `None` explicitly. |
 ...
 | `data/sample/README.md` | Documentation | Sample README uses `ZoneConfiguration` (not present in `models.py`) and includes examples that don’t match the current public API. | Medium | Update sample README examples to use `ZoneConfig` and keep sample data docs in sync with released API. |
 | `CHANGELOG.md` | Documentation | Changelog claims `ActivityData` and `ZoneConfiguration` models, and “OpenWeatherMap API” integration; these do not match the current codebase (no `ActivityData`, `ZoneConfig`, Open-Meteo). It also claims “85% code coverage”/“complete API documentation” which is not currently validated. | High | Treat the changelog as a release contract: align entries with current API + integrations (or mark claims as aspirational). |
 | `CITATION.cff` | Release Metadata | Repository URLs are placeholders (`yourusername`) and author metadata is generic. | Low | Replace placeholders with real repository links and author metadata before public release. |
 | `LICENSE` | Release Metadata | MIT license is present and matches `pyproject.toml`, but license/attribution were not audited against dependency licenses. | Low | Verify license compatibility for intended distribution and ensure attribution is accurate for public release. |
 | `docs/DATA_PREPARATION.md` | Privacy / Reproducibility | Doc states absolute timestamps should never be published, but `tools/sanitize_gps.py` does not scrub/normalize timestamps. Doc also assumes Parquet workflows and `data/sanitized/` outputs not present in the repo. | High | Align the tool + doc: ensure sanitization removes/normalizes timestamps, document Parquet engine dependency, and clarify expected data folders/artifacts. |
 | `reports/01_longitudinal_study.md` | Scientific Reporting | Report claims `data/processed/weekly_metrics.parquet (repository)` exists, but `data/processed/` is empty; narrative claims are not traceable to committed scripts+data. | High | Update report to reference actual committed data/scripts (or explicitly note what is intentionally omitted for privacy). |
...
- [x] **Release / Supporting Docs**
    -   **Data Preparation Guide:** Reviewed for alignment with sanitizer + repo structure (see findings log).
    -   **Study Report:** Reviewed for traceability to committed artifacts (see findings log).
    -   **Release Metadata:** Reviewed `CHANGELOG.md`, `CITATION.cff`, and `LICENSE` for accuracy/placeholder values (see findings log).
...
- **P1 (Reliability + reproducibility)**
    ...
    -   Align supporting docs + release metadata (`docs/DATA_PREPARATION.md`, `reports/01_longitudinal_study.md`, `CHANGELOG.md`, `CITATION.cff`, `data/sample/README.md`) with the actual codebase and committed artifacts.
